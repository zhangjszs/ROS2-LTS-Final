...
PurePursuitParams::PurePursuitParams(rclcpp::Node::SharedPtr node) {
    ...
    // 话题配置 (C++20 指定初始化器)
    topics =
        Topics{.path = get("topics.path", std::string("/planning/pathlimits")),
    ...
    system = System{.control_rate = control_rate,
    ...
    algorithm =
        Algorithm{.steering = {.delta_max = get("algorithm.steering.delta_max", 0.4),
                               ...
                               .mapping = {.units_per_degree = get("algorithm.steering.mapping.units_per_degree", 1.0),
                                           .neutral = get("algorithm.steering.mapping.neutral", 90),
                                           .min_raw = get("algorithm.steering.mapping.min_raw", 65),
                                           .max_raw = get("algorithm.steering.mapping.max_raw", 115)}},
                  .throttle = {.target_speed = get("algorithm.throttle.target_speed", 2.0),
                  ...
                  .path_search = {.search_window = get("algorithm.path_search.search_window", 50),
